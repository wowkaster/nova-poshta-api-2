...
class NovaPoshtaApi2
{
	...
	function getWarehouseByRef($cityRef, $warehouseRef)
	{

		$warehouses = $this->getWarehouses($cityRef);
		if (is_array($warehouses['data'])) {
			foreach ($warehouses['data'] as $warehouse) {
				if (mb_stripos($warehouse['Ref'], $warehouseRef) !== FALSE)
					return $warehouse;
			}
		}
	...
	function newInternetDocument($sender, $recipient, $params)
	{
		...
		if (empty($recipient['Warehouse'])) {
			$recipient["NewAddress"] = 1;
			/*			$recipient["RecipientAddressName"]: "",
